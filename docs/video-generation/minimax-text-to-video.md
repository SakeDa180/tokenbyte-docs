...
使用 TokenByte 调用 Minimax 文生视频接口

`POST` `https://api.tokenbyte.ai/api/v1/video/generation`

使用 TokenByte 统一视频任务接口调用 Minimax 的文生视频能力。模型名称请查看TokenByte支持的所有[AI 模型](https://tokenbyte.ai/models/)，不要根据厂商官网名称自行推测。
...
## 模式说明
Minimax 文生与图生共享任务端点，图生模式需要首帧图片。
