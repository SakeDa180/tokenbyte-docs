...
使用 TokenByte 调用 HappyHorse 参考生视频接口

`POST` `https://api.tokenbyte.ai/api/v1/video/generation`

使用 TokenByte 统一视频任务接口调用 HappyHorse 的参考生视频能力。模型名称请查看TokenByte支持的所有[AI 模型](https://tokenbyte.ai/models/)，不要根据厂商官网名称自行推测。
...
| 参数 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `model` | string | 是 | HappyHorse 模型的完整 ID |
...
## 模式说明
HappyHorse 根据文生、首帧和参考素材模式使用不同输入字段。
